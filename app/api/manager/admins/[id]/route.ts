...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (body.status) {
      const admins = await setAdminStatus(id, city, body.status);
      return NextResponse.json({ admins });
    }
    if (body.resetPassword) {
      await resetAdminPassword(id, city, body.resetPassword);
      return NextResponse.json({ ok: true, admins: await listAdminAccounts(city) });
    }
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const city = session.city || session.name;
  if (!Number.isFinite(id)) return NextResponse.json({ error: "Некорректный id администратора." }, { status: 400 });
  const admins = await deleteAdminAccount(id, city);
  return NextResponse.json({ admins });
}
