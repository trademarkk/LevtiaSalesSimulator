...
async function buildEvaluationPrompt(scenario: ScenarioContext, objections: ObjectionRecord[], messages: ChatMessage[], trainingState?: TrainingState) {
  ...
  return [
    "Ты наставник по продажам студии LEVITA.",
    ...
    "Строго придерживайся шаблона отчёта из контекста тренажера. Не возвращай старую числовую шкалу и не добавляй блок 'Оценка администратора (0-10)'.",
    "",
    "Контекст тренажера:",
    basePrompt,
    ...
    `- Как решала вопрос логистики по ${scenario.city || "городу"}/между студиями: …`,
    "- Цели/страхи: …",
...
function buildFallbackEvaluation(messages: ChatMessage[], scenario: ScenarioContext) {
  ...
  return [
    "🎯 Итог:",
    "- Решение: отказалась (до покупки не дошло; абонемент действует во всех студиях города, онлайн — дополнительная опция внутри абонемента).",
    "- Причины: • не все реальные сомнения были раскрыты • аргументы звучали слишком общо • не случился уверенный переход к конкретному следующему шагу",
    "",
    "🧩 Карта навыков (без суммы и без баллов — это снимок именно этой ситуации):",
    "Статусы:",
    "- Уверенно — было конкретно, уместно и двигало к решению",
    ...
    "2) Работа с возражениями — Следующий шаг",
    "3) Аргументация выгоды — Следующий шаг",
    "4) Удержание приоритета 144/96 (этика) — Следующий шаг",
    "5) Перевод к шагу оплаты/брони — Следующий шаг",
    "",
    "🧠 Комментарий (максимально прикладной):",
    "- Что уже получилось (минимум 2 пункта): • диалог не оборвался после первого сомнения • была попытка поддерживать контакт и двигать разговор дальше",
    "- Что усилить в следующий раз (1–3 пункта, как действия 'спросить/уточнить/сказать/предложить'): • глубже уточнить истинную причину сомнения • давать аргумент именно под ситуацию клиентки • завершать реплику конкретным следующим шагом",
    "- Главный фокус следующей тренировки (одно действие, которое даст максимум эффекта): после каждого возражения сначала уточнять суть, а не сразу отвечать общими фразами",
    `- Готовые формулировки (2–4 коротких варианта фраз, которые стоило сказать в этом диалоге): «Что для вас сейчас главный стоп-фактор — цена, время или уверенность, что будете ходить?», «Если подберем удобный формат и студию в ${scenario.city || "вашем городе"}, вам будет проще рассматривать абонемент?», «Давайте я коротко покажу разницу между 144 и 96 занятиями и что вам это даст по регулярности»`,
    `- Фразы администратора, которые сработали: ${managerPhrases.length ? managerPhrases.join(", ") : "«нужно выбрать 1–2 удачные реплики вручную, если диалог был слишком короткий»"}`,
    "",
    "🧍 Профиль клиента:",
    `- Возраст/профессия/график/доход/семья: ${scenario.persona}`,
    `- Случайное направление: ${scenario.lessonDirection}`,
    `- Как решала вопрос логистики по ${scenario.city || "городу"}/между студиями: смотрела бы на удобство дороги, пробки и близость студии к своим маршрутам`,
    "- Цели/страхи: есть интерес, но не хватает уверенности и ясности по формату",
    "- Использованные возражения (мин. 5): «нужно подумать», «дорого», «не уверена, что буду ходить регулярно», «может быть неудобно по времени», «пока не готова принять решение сразу»",
  ].join("\n");
}
